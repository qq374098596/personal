      ...
      <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-position="left" label-width="0px"
        ...
        <el-form-item prop="account">
          <el-input type="text" v-model="ruleForm.account" auto-complete="off" :placeholder="$t('login.account')"
                    id="loginEmail" @keyup.enter.native="handleSubmit"></el-input>
        </el-form-item>
        
        <el-form-item prop="checkPass">
          <el-input type="password" v-model="ruleForm.checkPass" auto-complete="off" :placeholder="$t('login.password')"
                    id="loginPassword" @keyup.enter.native="handleSubmit"></el-input>
        </el-form-item>
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        account: [
          {required: true, message: this.$t('login.inputAccount'), trigger: 'change'}
        ],
        checkPass: [
          {required: true, message: this.$t('login.inputPass'), trigger: 'change'}
        ]
      },
      checked: false
    }
  ...
  methods: {
    CookieSet: function(key, val, expiresDays){//这里传天数
      if (expiresDays){
        //要保存Cookie有效期
        var date = new Date();
    ...
    CookieGet: function(key){
      ...
      return unescape(unescape(res))//解码
//    return res
  	},
    handleSubmit (ev) {
    	this.logining = true
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          this.$axios.post('api/login/login/',loginParams,
