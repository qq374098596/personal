...
function action_log($action = null, $model = null, $record_id = null, $user_id = null){
    ...
    $data['action_ip']      =   ip2long('127.0.0.1');
    $data['model']          =   $model;
...
function parse_action($action = null, $self){
    ...
    foreach ($rules as $key=>&$rule){
        ...
        if (empty($return[1])) {
            if(!array_key_exists('cycle', $return[0]) || !array_key_exists('max', $return[0])){
                unset($return[0]['cycle'],$return[0]['max']);
            }
        }else{
...
function get_nickname($uid = 0){
    ...
    //             array_shift($list);
    //         }
    //         cacha('sys_user_nickname_list', $list);
    //     } else {
    //         $name = '';
    //     }
    // }
